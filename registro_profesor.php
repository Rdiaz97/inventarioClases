...
?>
    ...
    <h1 id="h1_registro_p">Bienvenido profesor, por favor identifiquese</h1>
    <section>
...
<?php 
    ...
    $correo=$_GET["email"]??null;
        if($registrar!=0){
            ...
            $sql_cedula="SELECT * FROM profesores WHERE cedula = '$cedula'";
            $res_ced=mysqli_query($conexion,$sql_cedula);
            $user= mysqli_fetch_assoc($res_ced); 
            if($user == null){
                echo "nadie se ha registrado aun";
                ...
                if($user["cedula"] != 0){
                    ...
                    echo $cedula."= ".$user["cedula"]."algo sucedio no hemos podido ingresar, asegura haber puesto bien tu cedula";
                }
            }else {
                echo "No coincide";
            }
